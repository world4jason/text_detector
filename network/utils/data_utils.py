...
def read_tfrecord(path, config, shuffle=True):
    ...
    decode_image = tf.cast(tf.image.resize_images(decode_image,config.IMAGE_SHAPE), tf.int32)

    y0_set = tf.cast(tf.sparse_tensor_to_dense(features['image/object/bbox/y0']), tf.float32)
    x0_set = tf.cast(tf.sparse_tensor_to_dense(features['image/object/bbox/x0']), tf.float32)
    y1_set = tf.cast(tf.sparse_tensor_to_dense(features['image/object/bbox/y1']), tf.float32)
    x1_set = tf.cast(tf.sparse_tensor_to_dense(features['image/object/bbox/x1']), tf.float32)
    y2_set = tf.cast(tf.sparse_tensor_to_dense(features['image/object/bbox/y2']), tf.float32)
    x2_set = tf.cast(tf.sparse_tensor_to_dense(features['image/object/bbox/x2']), tf.float32)
    y3_set = tf.cast(tf.sparse_tensor_to_dense(features['image/object/bbox/y3']), tf.float32)
    x3_set = tf.cast(tf.sparse_tensor_to_dense(features['image/object/bbox/x3']), tf.float32)
    cx_set = tf.cast(tf.sparse_tensor_to_dense(features['image/object/bbox/cx']), tf.float32)
    cy_set = tf.cast(tf.sparse_tensor_to_dense(features['image/object/bbox/cy']), tf.float32)
    w_set = tf.cast(tf.sparse_tensor_to_dense(features['image/object/bbox/w']), tf.float32)
    h_set = tf.cast(tf.sparse_tensor_to_dense(features['image/object/bbox/h']), tf.float32)
    label = tf.cast(tf.sparse_tensor_to_dense(features['image/object/class/label']), tf.float32)

    return_value = [decode_image,x0_set,y0_set,x1_set,y1_set,x2_set,y2_set,x3_set,y3_set,cx_set,cy_set,w_set,h_set,label]
    if shuffle:
